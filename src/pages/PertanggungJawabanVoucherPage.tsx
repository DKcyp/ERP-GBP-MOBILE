...
interface PertanggungJawabanVoucherPageProps {
  onBack: () => void;
}

interface VoucherResponsibilityEntry {
  no: number;
  noVoucher: string;
  noSo: string;
  noSoTurunan: string;
...
const voucherResponsibilityData: VoucherResponsibilityEntry[] = [
  {
    no: 1,
    noVoucher: 'VCH001',
    noSo: 'SO001',
  ...
  },
  {
    ...
    tglLaporan: '17-03-2025',
  },
];

const PertanggungJawabanVoucherPage: React.FC<PertanggungJawabanVoucherPageProps> = ({ onBack }) => {
  const [showEntries, setShowEntries] = useState(10);
  const [currentPage, setCurrentPage] = useState(1);
  const entriesPerPage = showEntries;

  const totalPages = Math.ceil(voucherResponsibilityData.length / entriesPerPage);
  const currentEntries = voucherResponsibilityData.slice(
  ...
  return (
    <div className="min-h-screen bg-background p-4 flex flex-col items-center">
      ...
      <div className="w-full max-w-md bg-surface p-6 rounded-xl shadow-card grid grid-cols-3 items-center mb-8 md:max-w-4xl">
        ...
        <h2 className="text-xl font-bold text-text text-center col-start-2">Pertanggung Jawaban Voucher</h2>
        ...
        <div className="bg-surface p-6 rounded-xl shadow-card flex flex-col gap-4 mt-4">
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[1600px] md:min-w-full"> {/* Adjust min-w for mobile scroll */}
              <div className="grid grid-cols-[0.5fr_1fr_1fr_1fr_1.5fr_1.2fr_1.5fr_1.5fr_1fr_1fr] gap-4 text-xs font-medium text-textSecondary uppercase py-2 border-b border-border">
                ...
                <span className="flex items-center">TGL PENGAJUAN VOUCHER <ArrowUp size={12} className="ml-1" /><ArrowDown size={12} /></span>
                <span className="flex items-center">TGL PEMBAYARAN VOUCHER <ArrowUp size={12} className="ml-1" /><ArrowDown size={12} /></span>
                <span className="flex items-center">TGL EXPIRED <ArrowUp size={12} className="ml-1" /><ArrowDown size={12} /></span>
                <span className="flex items-center">TGL LAPORAN <ArrowUp size={12} className="ml-1" /><ArrowDown size={12} /></span>
              </div>
              {currentEntries.map((entry, index) => (
                <div key={index} className="grid grid-cols-[0.5fr_1fr_1fr_1fr_1.5fr_1.2fr_1.5fr_1.5fr_1fr_1fr] gap-4 items-center py-3 border-b border-border/50 last:border-b-0">
                  ...
                  <span className="text-sm text-text">{entry.tglLaporan}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between mt-4 text-sm">
            <div className="text-textSecondary mb-2 sm:mb-0">
              Showing {(currentPage - 1) * entriesPerPage + 1} to {Math.min(currentPage * entriesPerPage, voucherResponsibilityData.length)} of {voucherResponsibilityData.length} entries
            </div>
...
export default PertanggungJawabanVoucherPage;
